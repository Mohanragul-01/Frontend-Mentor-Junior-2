...
const Extensions = ({ theme, selected }) => {
  const [extensionData, setExtensionData] = useState([]);

  const fetchData  = async () => {
    try {
      const result = await fetch("/data.json").then((response) => response.json())
    ...
    <>
      {filteredData.map((item) => (
        ...
        <div className="extension-actions">
          ...
          <Toggle theme={theme} isActive={item.isActive} onToggle={() => toggleSwitch(item.name)}/>
        </div>
      </div>
      ))}
    </>
